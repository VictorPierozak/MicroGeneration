...
CalledFromTauFactor=1; %Set to zero to trial
%% File imported
if nargin<5
...
if hand.InLineMode==0
    ...
    if RVA==0
        ...
        if length(NetVals)>2
            IA(1)=(SA(1)+SA(2)-SA(3))/2;
            IA(2)=(SA(2)+SA(3)-SA(1))/2;
            IA(3)=(SA(3)+SA(1)-SA(2))/2;
            assignin(hand.WoSpace,'temp2',IA*1e-6);
            evalin(hand.WoSpace,[varname,'.InterfaceAreaDens_over_um = temp2'';']);
            evalin(hand.WoSpace,'clear temp2');
        end
    else
    ...
    if length(NetVals)>2
        ...
        IA(2)=(SA(2)+SA(3)-SA(1))/2;
        IA(3)=(SA(3)+SA(1)-SA(2))/2;
        eval(['hand.',varname,'.InterfaceAreaDens_over_um = IA*1e-6;']);
...
if hand.InLineMode==0
    if RVA~=0
        ...
        if length(NetVals)>2
            ...
            ylim([roundsf(SA(end)/1e6*0.9,1,'floor') roundsf(SA(end)/1e6*1.1,1,'ceil')])
        ...
        if length(NetVals)>2
            ...
            ResultAnnno=annotation(RVfig(Mode),'textbox',...
                ...
                'String',{...
                [],['Voxel Size = ',num2str(roundsf(VoxDim(1)*1e9,4,'round')),' $\times$ ',num2str(roundsf(VoxDim(2)*1e9,4,'round')),' $\times$ ',num2str(roundsf(VoxDim(3)*1e9,4,'round')),' nm'],...
                [],['Total Voxels = ',num2str(a),' $\times$ ',num2str(b),' $\times$ ',num2str(c),' = ',num2str(a*b*c),' voxels'],...
                [],['Total Volume = ',num2str(roundsf(a*VoxDim(1)*1e6,3,'round')),' $\times$ ',num2str(roundsf(b*VoxDim(2)*1e6,3,'round')),' $\times$ ',num2str(roundsf(max(c,1)*VoxDim(3)*1e6,3,'round')),' $\mu$m = ',num2str(roundsf(a*b*c*prod(VoxDim)*1e9,3,'round')),' mm$^3$'],...
                [],['Volume Fractions:'],...
                ['    * Black = ',num2str(roundsf(VolFrac(1,1),3,'round'),'%.3f')],...
